...
function App() {
  ...
  const fetchData = () => {
    try {
      const getData = async () => {
        const getInitialData = await fetch(`https:geo.ipify.org/api/v2/country,city?apiKey=${import.meta.env.VITE_APP_API_KEY}&${checkIpAddress.test(ipAddress) ? `ipAddress=${ipAddress}` : checkDomain.test(ipAddress) ? `domain=${ipAddress}` : ""
      }`)
